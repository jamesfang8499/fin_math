...
\begin{frame}{首中时刻$\tau_a$分布函数的图示}
  \begin{tikzpicture}[>=latex, xscale=1.5, yscale=3]
    \fill[color=gray!70!white] [domain=-3:-1.25, samples=100, very thick]plot(\x, {exp(-.5*\x*\x)})--(-1.25,0)node[below, color=black]{$-\dfrac{|a|}{\sqrt{t}}$}--(-3,0);
    \fill[color=gray!70!white] [domain=1.25:3, samples=100, very thick]plot(\x, {exp(-.5*\x*\x)})--(3,0)--(1.25,0)node[below, color=black]{$\dfrac{|a|}{\sqrt{t}}$};
    \draw[->](-3.2,0)--(3.2,0)node[right]{$x$};
    \draw[->](0,-.3)--(0,1.3)node[right]{$f(x)$};
    \draw[domain=-3:3, samples=100, very thick]plot(\x, {exp(-.5*\x*\x)});
    \node at (0,0)[below left]{$O$};
    \node at (.3,1)[right]{$f(x)=\dfrac{1}{\sqrt{2\pi}}\exp\left[-\dfrac{1}{2}x^2\right]$};
    \end{tikzpicture}
\end{frame}
...
\begin{frame}{布朗运动最大值$M_t$分布函数的图示}
  \begin{tikzpicture}[>=latex, xscale=1.5, yscale=3]
    \fill[color=gray!70!white] [domain=-1.25:1.25, samples=100, very thick]plot(\x, {exp(-.5*\x*\x)})--(1.25,0)node[below, color=black]{$\dfrac{a}{\sqrt{t}}$}--(-1.25,0)node[below, color=black]{$-\dfrac{a}{\sqrt{t}}$};
...
\begin{frame}
  \frametitle{$M_t$与$W(t)$的联合概率密度}
  根据反射原理可知：当$w\le m$，且$m>0$时，下式成立：
  \begin{equation*}
    \Pr[M_t\ge m,\; W(t)\le w]=\Pr[W(t)\ge 2m-w]
  \end{equation*}
从而可以得到$M_t$与$W(t)$的联合概率密度的计算公式。
  
  \begin{block}{定理}
    $M_t$与$W(t)$的联合概率密度如下：
  \begin{equation*}
  f_{M,W}(m,w)=\frac{2(2m-w)}{t\sqrt{2\pi t}}\exp\left[-\frac{(2m-w)^2}{2t}\right],\qquad w\le m,\, m>0
  \end{equation*}
  \end{block}
\end{frame}  

\begin{frame}
  \frametitle{随机变量对$\left({M}_t, {W}(t)\right)$的取值区域}
\centering
  \begin{tikzpicture}[>=stealth, very thick]
    \fill[pattern=north west lines](1,-1.5)--(1,1)--(4,4)--(4,-1.5)--(1,-1.5);
    \node at (1,0)[below left]{$m$};
    \draw[->](-1,0)--(4.5,0)node[right]{$x$};
    \draw[->](0,-1.5)--(0,4)node[right]{$y$};
    \draw[thick](0,0)--(4,4)node[right]{$y=x$};
    \draw[dashed](1,1)--(0,1)node[left]{$m$};
    \end{tikzpicture}
